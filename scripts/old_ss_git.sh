...
yum install -y git aws-cli httpd cronie
echo "Git, AWS CLI, Apache, and Cronie installed"
...
# Set up a cron job to check for GitHub updates every 2 minutes
CRON_JOB="*/2 * * * * cd $WEB_DIR && /usr/bin/git fetch --all && /usr/bin/git reset --hard origin/main > /var/log/github_cron.log 2>&1"
(crontab -l 2>/dev/null | grep -v "git reset --hard" ; echo "$CRON_JOB") | crontab -
echo "Cron job added to check for updates every 2 minutes"
